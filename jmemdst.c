/*
 * jmemdst.c: memory buffer destination for compressed Jpeg data.
 * Modifications (C) 2010 Jamie Guinan.
 *
 * Derived from jdatadst.c, original copyright follows:
 *
...
static void dest_output(my_dest_ptr dest, int length)
{
  ...
  memcpy(dest->output_buffer + dest->output_offset, dest->local_buffer, length);
  dest->output_offset += length;
}
...
empty_output_buffer (j_compress_ptr cinfo)
{
  ...
  dest_output(dest, OUTPUT_BUF_SIZE);
...
term_destination (j_compress_ptr cinfo)
{
  ...
  if (datacount > 0) {
    dest_output(dest, datacount);
  }

  /* Store final length to user-supplied integer. */
  if (dest->jpeg_length) {
    *dest->jpeg_length = dest->output_offset;
  }
}
 ...
 * for freeing it after finishing compression.
 */
