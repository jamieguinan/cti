...
/* Xxx Instance and Template implementation. */
enum { INPUT_CONFIG, INPUT_JPEG };
static Input Xxx_inputs[] = { 
  [ INPUT_CONFIG ] = { .type_label = "Config_msg", .handler = Config_handler },
  [ INPUT_JPEG ] = { .type_label = "Jpeg_buffer", .handler = Jpeg_handler },
};

enum { OUTPUT_RGB3, OUTPUT_GRAY, OUTPUT_YUV422P, OUTPUT_JPEG };
static Output Xxx_outputs[] = { 
...
{
  Xxx_private *priv = (Xxx_private *)pi;
...
static Config config_table[] = {
  { "max_messages", 0L, 0L, 0L, cti_set_int, offsetof(Xxx_private, max_messages) },
  { "every", 0L, 0L, 0L, cti_set_int, offsetof(Xxx_private, every) },
...
static void Jpeg_handler(Instance *pi, void *data)
{
  ...
  if (priv->max_messages && pi->pending_messages > priv->max_messages) {
    ...
    fprintf(stderr, "Xxx skipping %d (%d %d)\n", pi->counter,
  ...
  if (pi->outputs[OUTPUT_YUV422P].destination
    ...
    while (pi->outputs[OUTPUT_YUV422P].destination->parent->pending_messages > 25) {
...
static void Xxx_tick(Instance *pi)
{
  Handler_message *hm;
...
static void Xxx_instance_init(Instance *pi)
{
  Xxx_private *priv = (Xxx_private *)pi;
...
static Template Xxx_template = {
  ...
  .outputs = Xxx_outputs,
  .num_outputs = table_size(Xxx_outputs),
  .tick = Xxx_tick,
  .instance_init = Xxx_instance_init,
};


void Xxx_init(void)
{
  Template_register(&Xxx_template);
}
