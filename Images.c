...
YUVYUV422P_buffer * YUVYUV422P_buffer_new(int width, int height, Image_common *c)
{
  YUVYUV422P_buffer * y422p = Mem_calloc(1, sizeof(*y422p));
  if (c) {
    y422p->c = *c;
  }
  y422p->width = width;
...
YUVYUV422P_buffer *YUVYUV422P_buffer_from(uint8_t *data, int len, Image_common *c)
{
  int i;
  ...
  if (width) {
    y422p = YUVYUV422P_buffer_new(width, height, 0L);
...
void YUVYUV422P_buffer_discard(YUVYUV422P_buffer *y422p)
{
...
void YUVYUV422P_buffer_unref(YUVYUV422P_buffer **y422p)
{
...
static void YUVYUV422P_to_xGx(YUVYUV422P_buffer *y422p, RGB3_buffer *rgb, BGR3_buffer *bgr)
{
  // R = Y + 1.402 (Cr-128)
...
RGB3_buffer *YUVYUV422P_to_RGB3(YUVYUV422P_buffer *y422p)
{
  RGB3_buffer *rgb = RGB3_buffer_new(y422p->width, y422p->height, &y422p->c);
  YUVYUV422P_to_xGx(y422p, rgb, 0L);
  return rgb;
}


BGR3_buffer *YUVYUV422P_to_BGR3(YUVYUV422P_buffer *y422p)
{
  BGR3_buffer *bgr = BGR3_buffer_new(y422p->width, y422p->height, &y422p->c);
  YUVYUV422P_to_xGx(y422p, 0L, bgr);
  return bgr;
}


YUVYUV422P_buffer *RGB3_toYUVYUV422P(RGB3_buffer *rgb)
{
  ...
  int x, y;
  YUVYUV422P_buffer *y422p = YUVYUV422P_buffer_new(rgb->width, rgb->height, &rgb->c);
...
YUVYUV422P_buffer *BGR3_toYUVYUV422P(BGR3_buffer *bgr)
{
  //Y = 0.299R + 0.587G + 0.114B
  //U'= (B-Y)*0.565
  //V'= (R-Y)*0.713
  ...
  int x, y;
  YUVYUV422P_buffer *y422p = YUVYUV422P_buffer_new(bgr->width, bgr->height, &bgr->c);
...
Y420P_buffer *YUVYUV422P_to_Y420P(YUVYUV422P_buffer *y422p)
{
...
YUVYUV422P_buffer *YUVYUV422P_copy(YUVYUV422P_buffer *y422p, int xoffset, int yoffset, int width, int height)
{
  YUVYUV422P_buffer *newbuf = 0L;
  int x, y;
  ...
  newbuf = YUVYUV422P_buffer_new(width, height, &y422p->c);
...
YUVYUV422P_buffer *YUVYUV422P_clone(YUVYUV422P_buffer *y422p)
{
  return YUVYUV422P_copy(y422p, 0, 0, y422p->width, y422p->height);
}


void YUVYUV422P_paste(YUVYUV422P_buffer *dest, YUVYUV422P_buffer *src, int xoffset, int yoffset, int width, int height)
{
  int x, y;
